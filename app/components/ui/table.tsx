...
export function TableHeader({ className, ...props }: React.HTMLAttributes<HTMLTableSectionElement>) {
  return <thead className={cn("[&_tr]:border-b bg-slate-50/75 border-slate-200 text-slate-600 font-medium text-xs uppercase tracking-wider", className)} {...props} />;
}
...
export function TableHead({ className, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th
      className={cn("h-11 px-4 text-left align-middle font-semibold text-slate-700", className)}
      {...props}
    />
  );
}
